...
HEIGHT = 32
data_structures = ["LSM", "BTree", "Warp", "vanEB5", "vanEB6", "Group1", "Group2", "Group3"]
...
showing_forms = {"LSM": {"marker": "8", "color": "C0"},
                 "BTree": {"marker": "^", "color": "C6"},
                 "Warp": {"marker": "v", "color": "C1"},
                 "vanEB5": {"marker": "8", "color": "C0"},
                 "vanEB6": {"marker": "*", "color": "C2"},
                 "Group1": {"marker": "o", "color": "C3"},
                 "Group2": {"marker": "o", "color": "C4"},
                 "Group3": {"marker": "o", "color": "C5"},
                 }
METHOD = "GetWarp"
...
def draw(ax, liste, name, data):
    ...
    print(ys)"""
    ys = list(map(lambda pairs: ((2**pairs[0]) / (pairs[1] * (10**-6))), zip(xs, ys)))
    #ys = list(map(lambda pairs: ((pairs[1] * (10**-6)) / (2**pairs[0])), zip(xs, ys)))
    #ys = list(map(lambda k: math.log(k, 2), ys))
    ax.plot(xs, ys, **data, label=name)
    #ax.scatter(xs, ys, s=get("sd", liste))

print(METHOD + "," + ",".join(data_structures))
f = next(iter(total.values()))
g = next(iter(f.values()))
...
ax = plt.subplot()
for key, value in total[METHOD].items():
    if key.startswith("vanEB") or key.startswith("Group"):
        continue
    print(key, value)
    draw(ax, value, showing_names[key], showing_forms[key])
print(total.keys())
for key, value in total["GetWorstWarp"].items():
    if key == "LSM":
        draw(ax, value, "LSM worst", {"marker": "8", "color": "C7"})
ax.legend()
plt.xlabel("Number of searches (2^i)")
plt.ylabel("Search rate (10^8 M ops/s)")
plt.title("{} - Comparison".format("Search per warp"))
plt.show()
