...
def extract_experience_from_file(data):
    regex = r"\[ RUN      \] (\w+)(Insertion|GetWorstThread|GetThread|PredecessorWarp|PredecessorWorstWarp|Predecessor|Successor|GetWorstWarp|GetWarp)(Thread)?Fixture\.\w+ \(\d+ runs, \d+ iterations per run\)"
    matches = re.finditer(regex, data)
    results = []
    for match in matches:
