...
//
//using XTrie = XFastTrieBinary<unsigned char, int>;
//using XTrieKey = typename XTrie::key_type;
//using XTrie3 = XFastTrieBinary<unsigned char, int, 3>;
//using XTrie3Key = typename XTrie3::key_type;
//using BigXTrie = XFastTrieBinary<int, int>;
//using BigXTrieKey = typename BigXTrie::key_type;
//using HugeXTrie = XFastTrieBinary<gpu::UInt64, int>;
//using HugeXTrieKey = typename HugeXTrie::key_type;
//
//SCENARIO("X-FAST-TRIE-BINARY", "[XFASTTRIE][BINARY]")
//{
//	int memory_size_allocated = 8u * 1024 * 1024;
