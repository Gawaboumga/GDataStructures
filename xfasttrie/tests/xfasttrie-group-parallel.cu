...
//
//using XTrie = XFastTrieGroupParallel<unsigned char, int>;
//using XTrieKey = typename XTrie::key_type;
//using XTrie3 = XFastTrieGroupParallel<unsigned char, int, 3>;
//using XTrie3Key = typename XTrie3::key_type;
//using MediumXTrie = XFastTrieGroupParallel<int, int, 16>;
//using MediumXTrieKey = typename MediumXTrie::key_type;
//using BigXTrie = XFastTrieGroupParallel<int, int>;
//using BigXTrieKey = typename BigXTrie::key_type;
//using HugeXTrie = XFastTrieGroupParallel<gpu::UInt64, int>;
//using HugeXTrieKey = typename HugeXTrie::key_type;
//
//SCENARIO("X-FAST-TRIE-GROUP-PARALLEL", "[XFASTTRIE][GROUPPARALLEL]")
//{
...
//	}
//
//	GIVEN("A X-fast trie for 2^16")
//	{
//		auto d_xtrie = cuda::memory::device::make_unique<MediumXTrie>(current_device);
//		cuda::launch(
//			initialize_allocator<MediumXTrie>,
//			{ 1u, number_warps * 32u },
//			d_allocator.get(), d_memory.get(), memory_size_allocated, d_xtrie.get()
//		);
//
